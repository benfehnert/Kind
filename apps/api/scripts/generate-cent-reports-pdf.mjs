...
async function main() {
  ...
  const templatePath = path.join(__dirname, "../src/templates/cent-reports/report-bundle.html");
  ...
  html = applyThemeToReportHtml(html, "morning-rules");

  const outDir = path.join(__dirname, "../output/cent-morning-rules-anna");
  mkdirSync(outDir, { recursive: true });
  const htmlOut = path.join(outDir, "reports-preview.html");
  const pdfOut = path.join(outDir, "kind-cent-morning-rules-anna-reports.pdf");
  const pdfDownloads = path.join(homedir(), "Downloads", "kind-cent-morning-rules-anna-reports.pdf");
...
`;
  ...
  const runner = path.join(tmpDir, "run-cent-pdf.cjs");
