import { MIN_COHORT_SIZE, HABIT_LABELS, HEALTH_EXPLORATION_LABEL, PRIMARY_OUTCOME } from "../constants.js";
...
export function generateCohortComparison(userEntries, currentWeek, cohortSnapshot, studyMeta) {
  ...
  for (let w = 1; w <= currentWeek; w += 1) {
    ...
    const c = cohortSnapshot.weekly_composure?.[String(w)] ?? cohortSnapshot.weekly_composure?.[w];
  ...
  const communityTopHabit = cohortSnapshot.top_habit_by_stress ?? null;
  ...
  return {
    ...
    adherence_comparison: {
      ...
      your_practice_adherence_percentage: userAdherence.intervention_adherence_pct,
