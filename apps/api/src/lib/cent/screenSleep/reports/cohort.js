...
export function generateCohortComparison(userEntries, currentWeek, cohortSnapshot, studyMeta) {
  ...
  for (let w = 1; w <= currentWeek; w += 1) {
    ...
    const c = cohortSnapshot.weekly_sleep?.[String(w)] ?? cohortSnapshot.weekly_sleep?.[w];
  ...
  const communityTopHabit = cohortSnapshot.top_habit_by_sleep ?? null;
  const habitMatch =
    userTop && communityTopHabit && userTop.habit === communityTopHabit;
  ...
  return {
    ...
    adherence_comparison: {
      ...
      your_screen_habit_adherence_percentage: userAdherence.intervention_adherence_pct,
