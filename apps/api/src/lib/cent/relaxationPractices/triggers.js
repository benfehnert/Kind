import { PHASE_PRACTICES, PHASE_OPTIMISE, PHASE_OUTPUT } from "./constants.js";
...
const COHORT_WEEKS = [7, 14, 21, 28, 35, 42];
...
export function analyzeRelaxationPractices(allEntries, studyMeta, cohortSnapshot = null) {
  const reports = [];
  const baselineEntries = allEntries.filter((e) => e.phase === "BASELINE");
  ...
  for (const week of [2, 4, 6]) {
    if (cohortSnapshot) {
      const cohort = generateCohortComparison(
        allEntries.filter((e) => e.study_week <= week * 7),
        week,
        cohortSnapshot,
        studyMeta
      );
